...
```{r}
hist(v, breaks = 20, col = "white", border = "black")
abline(v = mean(v), col = "red")

abline(v = median(v), col = "red", lty = 2)

abline(v = mean(v) - sd(v), col = "blue")

abline(v = median(v) - mad(v), col = "blue", lty = "dashed")
...
```{r}
set.seed(1)
z <- rnorm(n = 100000000, mean = 10, sd = 5)
hist(z)
```
...
```{r}
wilcox_result <- wilcox.test(x = 1:10, y = 5:14)
print(wilcox_result)
str(wilcox_result)
...
```{r}
t_result <- t.test(gene_exprs~group, df)
...
gene_table$gene_name[gene_table$gene_id==row.names(logcounts_matrix)
                     [which.min(result_bh)]]
